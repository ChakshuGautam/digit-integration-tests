...
test.describe('00-smoke: API helpers reach naipepea', () => {
  test('login returns token', { tag: ['@area:pgr', '@kind:lifecycle', '@kind:smoke', '@layer:api', '@persona:cross'] }, async () => {
    const auth = await loginEmployee();
    expect(auth.token).toBeTruthy();
  });

  test('mdms search returns Department schema records', { tag: ['@area:pgr', '@kind:lifecycle', '@kind:smoke', '@layer:api', '@persona:cross'] }, async () => {
    const auth = await loginEmployee();
  ...
  test('pgr search round-trips a known CLOSEDAFTERRESOLUTION complaint', { tag: ['@area:pgr', '@kind:lifecycle', '@kind:smoke', '@layer:api', '@persona:cross'] }, async () => {
    const auth = await loginEmployee();
  ...
  test('hrms employee search returns >0 LMEs', { tag: ['@area:pgr', '@kind:lifecycle', '@kind:smoke', '@layer:api', '@persona:cross'] }, async () => {
    const auth = await loginEmployee();
    const r = await hrmsSearch(auth, 'ke.nairobi', ['PGR_LME']);
    expect(r.Employees?.length).toBeGreaterThan(0);
  });

  test('PGR business service is present', { tag: ['@area:pgr', '@kind:lifecycle', '@kind:smoke', '@layer:api', '@persona:cross'] }, async () => {
    const auth = await loginEmployee();
